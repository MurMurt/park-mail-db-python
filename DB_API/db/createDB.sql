...
CREATE TRIGGER voteUpdate AFTER UPDATE ON vote FOR EACH ROW EXECUTE PROCEDURE vote_update();
...
CREATE TRIGGER voteInsert AFTER INSERT ON vote FOR EACH ROW EXECUTE PROCEDURE vote_insert();

CREATE OR REPLACE FUNCTION post_insert() RETURNS TRIGGER AS
  $BODY$
    BEGIN
      UPDATE post SET parent_id=new.id WHERE id=new.id AND parent_id ISNULL ;
  ...
  LANGUAGE plpgsql;
-- --
CREATE TRIGGER postInsert AFTER INSERT ON post FOR EACH ROW EXECUTE PROCEDURE post_insert();